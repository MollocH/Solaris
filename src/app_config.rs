...
pub fn read() -> AppConfig {
    ...
    let f = File::open(app_config_file).expect(&format!("Could not open file '{}'", app_config_file));
    return serde_yaml::from_reader(f).expect(&format!("Could not parse yaml file '{}'", app_config_file))
}